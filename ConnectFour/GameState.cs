...
public class Move
{
    ...
    public Move(int player, int column, int row, int turnNumber)
    {
        Player = player;
        Column = column;
        Row = row;
...
public class GameState
{
    ...
    public byte PlayPiece(int column)
    {
        ...
        var move = new Move(PlayerTurn, column, landingRow, CurrentTurn + 1);
        MoveHistory.Add(move);

        // Notify components that a move was added
        OnMoveAdded?.Invoke();
